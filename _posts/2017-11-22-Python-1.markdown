...
Affichez le nombre $$\pi$$ avec 

{% highlight python %}
math.pi
{%endhighlight%}

On obtient la racine carrée avec 

{% highlight python %}
math.sqrt(<nombre>)
{%endhighlight%}

On donne aussi que $$\cos (2n\pi) = \cos(0) = 1$$ pour tout entier relatif $$n$$.

Le vérifier avec Python sur quelques nombres que vous choisirez.

Etablir une propriété similaire avec $$\sin$$ et le tester. Des différences ?
